...
class UV_Map_Generator():
    def __init__(self, UV_height, UV_width=-1, 
        ...
        if not os.path.isfile(UV_pickle):
            ...
            self.v_to_vt = [list(v) for v in self.v_to_vt]
            # self.vt_to_v = [list(v) for v in self.vt_to_v]
            # import ipdb;ipdb.set_trace()
            max_v = 0
            for v in self.v_to_vt:
                if max(v) > max_v:
                    max_v = max(v)
            v_to_vt = np.zeros((max_v + 1, len(self.v_to_vt)), dtype=np.float32)
            for idx, v in enumerate(self.v_to_vt):
                for item in v:
    ...
    def _parse_obj(self, obj_file, cache_file):
        ...
        # to vertex faces
        vt_faces = []
        self.vt_to_v = [None] * self.texcoords.shape[0]
        ...
        self.v_to_vt = np.array([list(v) for v in self.v_to_vt])
        self.vt_to_v = np.array([list(v) for v in self.vt_to_v])
    ...
    def resample(self, UV_map):
        ...
        # import ipdb;ipdb.set_trace()
        # new_vt_3d = [i if i is not None else torch.tensor([0, 0,0]) for i in vt_3d]
        # vt_3d = torch.stack(new_vt_3d).to(device)
        # convert vt back to v (requires v_to_vt index)
        # self.v_to_vt = self.v_to_vt
        # cyka_v_3d = [None] * len(self.v_to_vt)
        # for i in range(len(self.v_to_vt)):
        #     cyka_v_3d[i] = torch.mean(vt_3d[
        return cyka_v_3d
