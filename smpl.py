...
class VertexJointSelector(nn.Module):
    ...
    def h36m_forward(self, vertices):
        joints_h36m17 = torch.einsum('bik,ji->bjk', [vertices, self.J_regressor_h36m17])
        return joints_h36m17
...
class SMPL(nn.Module):
    ...
    def h36m_joints_extract(self, vertices):
        if isinstance(vertices, np.ndarray):
            vertices = torch.from_numpy(vertices).type(self.dtype)
        joints_h36m17 = self.vertex_joint_selector.h36m_forward(vertices)
        return joints_h36m17
...
def test_smpl(smpl_model, dtype=torch.float32):
    ...
    for _ in range(200):
        ...
        verts_np = outputs[0].cpu().numpy()
        # import ipdb; ipdb.set_trace()
        pcd = open3d.geometry.PointCloud()
        # pcd.points = open3d.utility.Vector3dVector(verts_np[0])
        joints = outputs[1].cpu().numpy()
        pcd.points = open3d.utility.Vector3dVector(joints[0])
        color = np.ones_like(joints[0])
        color[:,1:] = 0
        pcd.colors = open3d.utility.Vector3dVector(color)
        
        pcd2 = open3d.geometry.PointCloud()
        pcd2.points = open3d.utility.Vector3dVector(verts_np[0])
        visualization.draw_geometries([pcd, pcd2])
        exit()
...
def prepare_smpl_model(dtype):
    model_path = './ROMP_SMPL/SMPL_NEUTRAL.pth'
